// ============================================================
// Static JSON generated by GitHub Actions (compute prices only)
...
// ---------- Pricing constants (storage) ----------
// AWS storage per-GB-month (defaults; region-dependent; tune later)
const AWS_STORAGE = {
  // General Purpose SSD (gp3) per GB-month
  ssd_per_gb_month: 0.08,
  // Throughput Optimized HDD (st1) per GB-month
  hdd_per_gb_month: 0.045
};

// Azure monthly prices by size (GiB) from your screenshots.
// Standard SSD (E-series): E1/E2/E3/E4/E6/E10/E15/E20
const AZURE_SSD_MONTHLY = {
   4: 0.300,
   8: 0.600,
  16: 1.200,
  32: 2.400,
...
// Standard HDD (S-series): S4/S6/S10/S15 from your screenshot.
// S20 (512GiB) left null for now; we’ll pull exact price via Actions later.
const AZURE_HDD_MONTHLY = {
   32: 1.536, // S4
   64: 3.008, // S6
  128: 5.888, // S10
  256: 11.328, // S15
  512: null   // S20 → to be filled later
};
...
async function compare() {
  ...
  try {
    ...
    // ---------- STORAGE PRICING ----------
    // AWS storage price (per-GB-month)
    const awsStorageMonthly = getAwsStorageMonthly(storageType, storageAmtGB);     // $/month
    const awsStorageHr      = (awsStorageMonthly != null) ? awsStorageMonthly / HRS_PER_MONTH : null;

    // Azure storage price (fixed SKU/month)
    const { sku: azDiskSku, size: azDiskGB, monthlyUSD: azStorageMonthly } =
      getAzureStorageSkuAndMonthly(storageType, storageAmtGB);                     // $/month (may be null)
    ...
    // If we have a SKU mapping, show it next to the selection
    if (azDiskSku) {
...
// ============================================================
// Small helpers for safe DOM writes
...
function sumSafe(a, b) {
  ...
  if (a == null && b == null) return null; // both unknown → show "—"
  return na + nb;
}
...
function getAwsStorageMonthly(type, gb) {
  ...
  if (t === "ssd") return gb * AWS_STORAGE.ssd_per_gb_month;
  return gb * AWS_STORAGE.hdd_per_gb_month; // default to HDD (st1)
}

// Azure: map (type, gb) → SKU & monthly price (rounded up to nearest supported size)
function getAzureStorageSkuAndMonthly(type, gb) {
  const t = (type || "hdd").toLowerCase();
  if (!isFinite(gb) || gb <= 0) return { sku: null, size: null, monthlyUSD: null };

  if (t === "ssd") {
    // Exact sizes supported in the table: 4,8,16,32,64,128,256,512
    const size = nearestCeil(gb, Object.keys(AZURE_SSD_MONTHLY).map(Number));
    const monthlyUSD = AZURE_SSD_MONTHLY[size] ?? null;
    const sku = sizeToAzureSku("ssd", size);
    return { sku, size, monthlyUSD };
  } else {
    // HDD table supports: 32,64,128,256,(512 pending)
    const size = nearestCeil(gb, Object.keys(AZURE_HDD_MONTHLY).map(Number));
    const monthlyUSD = AZURE_HDD_MONTHLY[size] ?? null;
...
function nearestCeil(requested, allowedSizes) {
  ...
  return sorted[sorted.length - 1]; // cap to largest known
}
...
function sizeToAzureSku(type, size) {
  if (type === "ssd") {
    ...
    return map[size] ? `E${map[size].slice(1)}` : null;
  } else {
...
// ---------- Reset cards ----------
function resetCards() {
  // Keep the "Recommended ..." labels even before results
...
function findBestAzure(list, vcpu, ram, os) {
  ...
  for (const x of enriched) {
    ...
    if (isFinite(x.vcpu) && isFinite(x.ram)) {
      ...
      score = 500 + price; // prefer known specs
    }
...
function inferAzureCoresRamFromName(name) {
  ...
  // Rough family RAM per core defaults
