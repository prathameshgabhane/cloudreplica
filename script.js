...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // NEW: run an initial comparison so users see prices immediately
  compare();
});
...
async function compare() {
  ...
  if (btn) btn.disabled = true; // guard if button missing
  ...
  try {
    ...
    if (!awsCard || awsCard.error) {
      ...
      if (awsInst) awsInst.innerHTML = `<strong>Recommended Instance:</strong> Error: ${awsCard?.error ?? "No match"}`;
    } else {
      const awsInst = document.getElementById("awsInstance");
      if (awsInst) awsInst.innerHTML = `<strong>Recommended Instance:</strong> ${awsCard.instance} (${awsCard.region})`;
    ...
    if (!azCard || azCard.error) {
      ...
      if (azInst) azInst.innerHTML = `<strong>Recommended VM Size:</strong> Error: ${azCard?.error ?? "No match"}`;
    } else {
      const azInst = document.getElementById("azInstance");
      if (azInst) azInst.innerHTML = `<strong>Recommended VM Size:</strong> ${azCard.instance} (${azCard.region})`;
    ...
    // FIX: closing quote was a backtick earlier; corrected to a double-quote
